...
func (c *Chef) SetAddress(address string) {
	c.address = address
}

func (c Chef) CreateCookbook(indexOffset int, amount int) []sdk.Msg {

	msgs := []sdk.Msg{}

	for i := indexOffset; i < indexOffset+amount; i++ {
		msgs = append(msgs, &types.MsgCreateCookbook{Creator: c.address, Id: fmt.Sprintf("cb%v", i), Name: fmt.Sprintf("testCookBook%v", i), Description: "this is a cookbook", Developer: "ali", Version: "v1.0.0", SupportEmail: "e@email.com", Enabled: true})
	}
...
func (c Chef) CreateRecipes(indexOffset int, amount int, cookBookID string) []sdk.Msg {
	msgs := []sdk.Msg{}
	...
	for i := indexOffset; i < indexOffset+amount; i++ {
		msgs = append(msgs, &types.MsgCreateRecipe{Creator: c.address, CookbookId: cookBookID, Id: fmt.Sprintf("testRecipe%v%v", cookBookID, i), Name: fmt.Sprintf("testRecipeName%v", i), Description: "this is a recipe", Version: "v1.0.1", CoinInputs: []types.CoinInput{}, ItemInputs: []types.ItemInput{}, Entries: entries, Outputs: []types.WeightedOutputs{}, BlockInterval: 0, CostPerBlock: sdk.NewCoin("upylon", sdkmath.NewInt(0)), Enabled: false, ExtraInfo: "extraInfo"})
	}
...
func (c Chef) ExecuteRecipes(indexOffset int, amount int, cookBookID string, recipeId string) []sdk.Msg {
	msgs := []sdk.Msg{}

	for i := indexOffset; i < indexOffset+amount; i++ {
		msgs = append(msgs, &types.MsgExecuteRecipe{Creator: c.address, CookbookId: cookBookID, RecipeId: recipeId, CoinInputsIndex: 0, ItemIds: []string{}, PaymentInfos: []types.PaymentInfo{}})
	}
...
func (c Chef) CreateComplexRecipeEasel(cookBookID string, recipeID string) types.MsgCreateRecipe {
